...
do
    ...
    kitchen_area=$(awk -v min=5 -v max=30 'BEGIN { print min+rand()*(max-min) }')
    living_area=$(awk -v min=10 -v max=100 'BEGIN { print min+rand()*(max-min) }')
    total_area=$(awk -v min=20 -v max=150 'BEGIN { print min+rand()*(max-min) }')

    rooms=$((RANDOM % 5))
    build_year=$((RANDOM % 130 + 1900))
    building_type_int=$((RANDOM % 10 + 1))

    latitude=$(awk -v min=55.5 -v max=55.9 'BEGIN { print min+rand()*(max-min) }')
    longitude=$(awk -v min=37.3 -v max=37.9 'BEGIN { print min+rand()*(max-min) }')
    ceiling_height=$(awk -v min=2.0 -v max=4.0 'BEGIN { print min+rand()*(max-min) }')
